 ...
 */
 define(['jquery', 'jqueryui', 'core/str'], function($, jqui, str) {
  ...
  var buildstrings = function(s) {
    ...
    criterionHTML += '    <div class="operator">'+ strings.distributionMode +' : <select onChange="$(this).parent().prev().html(this.value);" style="margin-top:4px;">';
  ...
  var setup = function() {
    $(".stepper").each(function () {
      ...
      var reload    = "<span id='nbstudentsdsp'> / " + $(".student").length + '(' + Math.ceil($(".student").length / parseInt($("#nbteam").val())) + ')' +"</span>";
  ...
  var updateTeams = function(numTeams) {
    ...
    if ($('#id_namingscheme').val().indexOf('*') > -1) {
      ...
      for (var i = 0; i < numTeams; i++) {
        ...
        if ($( "#sumbox_"+i).data("name") === undefined || $( "#sumbox_"+i).data("name").indexOf('dataname') > -1 ) sFullTeamName = sFullTeamName.replace('*', '');          
        else sFullTeamName = sFullTeamName.replace('*', $( "#sumbox_"+i).data("name"));      
      ...
      if (teamNames.length > numTeams) {
        ...
        var aRand = [];
        for (i = 0; i < aTeams.length; i++) { aRand[i] = i;}
        aRand = shuffle(aRand);
  ...
  var updateRunningCounter = function(criterion) {
    ...
    switch(boolOp) {
      ...
      case strings.cluster:
        ...
        if ((ctr/nbteam) < 2) strText = strText + strings.analyzeclusterwarning
        else  strText = strText + strings.analyzeclustersuccess;
      break;
      // AGGREGATE
      case strings.aggregate:
        ...
        for(var j=0; j<aAnswer.length; j++) {
          ...
          if (reste != 0) {
            color='red';
            strText = strText + strings.analyzeaggregatewarning.replace('{color}',color).replace('{answer}',questions[c.question].answers[aAnswer[j]].answer).replace('{nbstudent}',lctr).replace('{reste}',reste).replace('{nbgroup}',nbAggTeam).replace('{nbstudentgroup}',studAvgByTeam).replace('{reste}',reste);
          } else {
            color='green';
            strText = strText + strings.analyzeaggregatewarningOK.replace('{color}',color).replace('{answer}',questions[c.question].answers[aAnswer[j]].answer).replace('{nbstudent}',lctr).replace('{reste}',reste).replace('{nbgroup}',nbAggTeam).replace('{nbstudentgroup}',studAvgByTeam).replace('{reste}',reste);
          }
        }          
      break;
      // DISTRIBUTE
      case strings.distribute:
        ...
        for(var j=0;j<aAnswer.length;j++) {
          ...
          if(ctr >= nbteam) status = strings.analyzedistributesuccess.replace('{nbteam}', nbteam); 
          else status = strings.analyzedistributewarning.replace('{nbteam}', nbteam); 
          strText = strText + strings.analyzedistributecriterion.replace('{answer}', questions[c.question].answers[aAnswer[j]].answer).replace('{nbstudent}', ctr).replace('{status}', status);
        }          
      break;
  ...
  var synchroniseViewToModel = function() {
    ...
    if(preventDeleteClick == true) $(".studentdel").hide();
    else $(".studentdel").show();
  };
  //*********************************************************************************************************************************************
  ...
  var synchroniseModelToView = function() {
    ...
    for (i in teamAssignments) {
      ...
      for (j in team) {
        ...
        if (myResponses) {
          for (k in questions) {
            ...
            // Check if the question is in aggregate mode and if the answer is the good one do not count other to that question...
            for (m in q.answers) {
              a = q.answers[m];
              if ($.inArray(parseInt(a.id), myResponses) != -1) {
                for(cmdIdx=0;cmdIdx < aCommand.length; cmdIdx++) {
                  if(aCommand[cmdIdx][0] == strings.aggregate && aCommand[cmdIdx][2] == q.id) {
                    if(typeof aTeamMaxAgg[i] !== 'undefined' && aTeamMaxAgg[i][cmdIdx] == aSum[k].answers[m].id) { // !!! to be verify !!!
        ...
        if ($.inArray(studentID, selectedStudents) != -1) {
          ...
          if (typeof gteamAssignmentsWeight[i] !== 'undefined') {
            //studentDiv.append(' (' + gteamAssignmentsWeight[i][j] + ')');
          }
        }
      }
      ...
      if(teamAssignments[0].length > 0) {
        gpok = 0;                                                           // This variable is positive if they've only students that match the criterion in the team
        message = message + "<div id='sumbox_"+i+"' data-name={dataname_"+i+"} class='{sumbox_"+i+"} {avgbox_"+i+"} col-sm-4 col-md-3 border-rounded-right mylittlebox'><span style='font-weight:bold;'>"+teamNames[i]+" ("+ team.length +")</span><br>";
        for (k in aSum) {
          q = aSum[k];
          failed = 0;
          values = 0;
          for (l in q.answers) { 
            ...
            if(color != '') message = message + aSum[k].question + " - " + aSum[k].answers[l].answer + " : <span style='"+color+"'>" + aSum[k].answers[l].count + "</span><br>";
          ...
          if(failed == 0  && values >0) {
            gpok++;
            message = message + '<b><span style="color:green">' + aSum[k].question + ' ' + strings.teamupsuccess + '</span></b><br>';
          } else if (values > 0) {
            gpok = -10000;
            message = message + '<b><span style="color:red">'   + aSum[k].question + ' ' + strings.teamupwarning + '</span></b><br>';
          } else {
            gpok++;
          }
        }
    ...
    if(teamAssignments.length > 0 && teamAssignments[0].length > 0) {
      ...
      colSuccess = 'red';
      if(nSuccess > 80)   colSuccess = 'orange';
      if(nSuccess == 100) colSuccess = 'green';
      
      message = message +'<div class="col-sm-4 col-md-3 border-rounded-right">';
      message = message + strings.nbGroupSuccess + ' : <span style="color:'+colSuccess+';">'+ groupeOK + '/' + teamNames.length + '</span>';
      message = message + '<br>' + strings.nbStudent + ' : '+ nbStudent;
      message = message + '</div>';
      if(typeof teamAvg[i] !== 'undefined') {
        message = message +'<div class="col-sm-4 col-md-3 border-rounded-right">'+ strings.average + ' : ' + average + '<br>' + strings.standarddeviation + ' :' + ecartType + '<br>'+ strings.bornes + ':' + (average-(ecartType/2)).toFixed(2) + '-' + (average+(ecartType/2)).toFixed(2) +'</div>';
        $('#feedback').html(strings.teamupsuccessnbr + ' : <span style="color:'+colSuccess+';">'+ groupeOK + '/' + teamNames.length + '</span><br>' + strings.nbStudent + ' : '+ nbStudent + '<br>' + strings.average + ' : ' + average + '<br>' + strings.standarddeviation + ' :' + ecartType + '<br>'+ strings.bornes +':' + (average-(ecartType/2)).toFixed(2) + '-' + (average+(ecartType/2)).toFixed(2));
      } else {
        $('#feedback').html(strings.teamupsuccessnbr + ' : <span style="color:'+colSuccess+';">'+ groupeOK + '/' + teamNames.length + '</span><br>' + strings.nbStudent + ' : '+ nbStudent);
      }
    } else {
      $('#feedback').html('');
    }
  ...
  var buildTeams = function(bReset, bPrettify) {
    ...
    for (c in criterionGroup) {
      ...
      if(aCommand[i][0] == strings.aggregate) {
        ...
        for (qa in questions[criterion['question']].answers) {
          aCommand[i][1] = aCommand[i][1] + qa + ',';
          $('#aggList').append($('<option>', { value: questions[criterion['question']].answers[qa].answer.split(' ').join('_'),text : questions[criterion['question']].answers[qa].answer }));
        }
        $('#aggList').css('display','');
    ...
    // ATTENTION the variable i is used inside each call 
      ...
      if(bPrettify) {                           // Compact group try that all groups have the same size
        ...
        for(z=0;z<10;z++) {
          var nMove = 0;
          for (j=0; j < teamAssignments.length; j++) {
            // team is too big
            if(teamAssignments[j].length > avgStudentGroup) {
              for(k=teamAssignments[j].length-1; k>=0;k--) {
                ...
                if(bAgg == true && aDontMove.reduce(function(n, val) {return n + (val === teamAssignments[j][k]);}, 0) == 1)  {
                  for (l=0; l < teamAssignments.length; l++) {
                    if(teamAssignments[l].length - teamAssignments[j].length < -1 && aTeamMaxAgg[l][qAgg] ==  aTeamMaxAgg[j][qAgg]) {
          ...
          if(nMove == 0) z = 15;                // No move no loop... go away ! break should work !
          gteamAssignmentsWeight = sortTeamByWeight();
        }
  ...
  //*********************************************************************************************************************************************
  // Execute the balance algorithm. A team must have an average 'score' near the global average more less the half of the standard deviation (écart type)
  //*********************************************************************************************************************************************
  var balance = function(bPrettify) {
    ...
    // Compute the standard deviation (Racine carré de la somme des carrées de la soustraction du score de l'étudiant-la moyenne gloabale divisé par le nombre d'étudiant)
    for (j=0; j < teamAssignments.length; j++) {                                // Loop teams
      for(k=0; k < teamAssignments[j].length; k++) {                            // Loop students
        for(l=0; l < aNeeded.length; l++) {                                     // Loop values
          if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[l],10)) != -1)  {
    ...
    for (j=0; j < teamAssignments.length; j++) {                                // Loop teams
      for(k=0; k < teamAssignments[j].length; k++) {                            // Loop students
        if(teamAvg[j] > average+(ecartType/2)) {                                // The average of team is higher than the average + 50% of the standard deviation
          for(l=0; l < aNeeded.length; l++) {                                   // Loop values
            if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[l],10)) != -1)  {
                ...
                if(oStudent.length > 0) {
                  ...
                  for(var m=0; m < teamAssignments[j].length; m++) {            // Loop students
                    for(var n=0; n < aNeeded.length; n++) {                     // Loop values
                      if(responses[teamAssignments[j][m]].length > 0 && responses[teamAssignments[j][m]].indexOf(parseInt(aNeeded[n],10)) != -1)  {
                  ...
                  for(var m=0; m < teamAssignments[oStudent[1]].length; m++) {  // Loop students
                    for(var n=0; n < aNeeded.length; n++) {                     // Loop values
                      if(responses[teamAssignments[oStudent[1]][m]].length > 0 && responses[teamAssignments[oStudent[1]][m]].indexOf(parseInt(aNeeded[n],10)) != -1)  {
                  ...
                  if(teamAvg[j] < average+(ecartType/2)) {                      // Problem solve
        ...
        } else if(teamAvg[j] < average-(ecartType/2)) {                         // The average of team is lower than the average - 50% of the standard deviation
          for(l=0; l < aNeeded.length; l++) {                                   // Loop values
            if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[l],10)) != -1)  {
              if(parseInt(questions[aCommand[i][2]].answers[parseInt(aNeeded[l],10)].answer,10) <  average) { // if student result is lower than the average find a heavier one...
  ...
  var aggregate = function(bPrettify) {
    for (j=0; j < teamAssignments.length; j++) {                                // Loop teams
      ...
      aTeamMaxAgg[j][i] = parseInt(aNeeded[maxValue],10);                       // Save the most representative value for the aggregation in the group
      for(k=0; k < teamAssignments[j].length; k++) {                            // Loop students for completing the most represented value
                                                                                // Check if the student don't have this value and can be swapped
        if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(aTeamMaxAgg[j][i]) == -1 && aDontMove.indexOf(teamAssignments[j][k]) == -1)  { 
          ...
          if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[maxValue],10)) != -1) {
            aDontMove.push(teamAssignments[j][k]);                            // Upgrade the weight of the already locked student with the right value
          }
        }
      }
      ...
      if(bPrettify == true) {                                                   // Try to prettify the group by moving values that are not the dominant
        ...
        for(k=0; k < teamAssignments[j].length; k++) {
          if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(aTeamMaxAgg[j][i]) == -1 && aDontMove.indexOf(teamAssignments[j][k]) == -1)  { // check if the student don't have this value and can be moved
            var nAnswer = -1;
  ...
  var distribute = function(bPrettify) {
    for (j=0; j < teamAssignments.length; j++) {                                // Loop teamss
      var aNeeded =  aCommand[i][1].split(",");                                 // List of needed value
      for(k=0; k < teamAssignments[j].length; k++) {                            // First loop inside a team to remove value that are already in locked student
      ...
      for(k=0; k < teamAssignments[j].length; k++) {                            // Second loop inside the team to find missing value to complete the distribution of 'skills' (values)
        if(aDontMove.indexOf(teamAssignments[j][k]) == -1) {                    // Can be remove the student are not locked
          nLength = aNeeded.length-1;
          for(l=nLength;l>=0;l--) {
            if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[l],10)) != -1) { // valueS find and can be removed
              aNeeded.splice(l, 1); 
      ...
      // Find missing values
      for(l=0; l < aNeeded.length; l++) {                                       // Try to find missing values in other teams
  ...
  var cluster = function(bPrettify) {
    ...
    for (j=0; j < teamAssignments.length; j++) {                                // Loop througth teams
      single = -1;                                                              // -1 mean no one is found
      nCount = 0;
      for(k=0; k < teamAssignments[j].length; k++) {                            // Loop inside a team to get student
        var valueArray = aCommand[i][1].split(",");                             // List of request value that cannot be single
        for(var l=0; l<valueArray.length; l++){
          if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(valueArray[l],10)) > -1) { 
            nCount++;
            single = k;                                                         // keep in memory the single student (he can be also not single !) it's just the last one found
            if(nCount<=2) aDontMove.push(teamAssignments[j][k]);                // Don't lock the student if they've already enougth to meet criteria
            break;
          }                       
        }
      ...
      if(nCount == 1) {                                                         // Try to swap or prettify to complete 0 is OK 2 or more is OK too but be alone is so sad
        if(bPrettify == true) {                                                 // Try to prettify the group by removing single (move)
          ...
          if(group > -1) {
            teamAssignments[group].push(teamAssignments[j][single]);            // Add the student to his new group (move not a swap)
            aDontMove.push(teamAssignments[j][single]);                         // LOCK IT (TEST)....
            teamAssignments[j].splice(single, 1);                               // Remove him from old group
          } else {                                                              // If no one was found remove the last student of locked list to free him because the criteria is not meet in that group
            if(bAgg == true) {
              ...
              aDontMove.pop();                                                   // If no one was found remove the last student of locked list to free him because the criteria is not meet in that group
            }
          }
        } else {                                                                // Usual algo try to swap
          var oStudent = findSwap(j, aCommand[i][1]);
          if(oStudent.length > 0) {
            for(k=0; k < teamAssignments[j].length; k++) {                      // Find a student in the team that can be swap
              if(aDontMove.indexOf(teamAssignments[j][k]) == -1) {              // Can be swap 
                swapStudent(j,k,oStudent);
                break;
              }
    ...
    for (var i=0; i < teamAssignments.length; i++) {
      if(i != team) { // don't examine the original team !
        for(var j=0; j < teamAssignments[i].length; j++) {
          ...
          for(var l=0; l<valueArray.length; l++) {
            if(responses[teamAssignments[i][j]].length > 0 && responses[teamAssignments[i][j]].indexOf(parseInt(valueArray[l],10)) > -1) {
  ...
  var findSwapAgg = function(team, findList) {
    for (var i=0; i < teamAssignments.length; i++) {
      if(i != team && typeof aTeamMaxAgg[0] !== 'undefined' && aTeamMaxAgg[i][qAgg] == aTeamMaxAgg[team][qAgg]) { // don't examine the original team !
        for(var j=0; j < teamAssignments[i].length; j++) {
          var valueArray = findList.split(",");
          for(var l=0; l<valueArray.length; l++) {
            if(responses[teamAssignments[i][j]].length > 0 && responses[teamAssignments[i][j]].indexOf(parseInt(valueArray[l],10)) > -1) {
              if(aDontMove.reduce(function(n, val) {return n + (val === teamAssignments[i][j]);}, 0) == 1) { // Can be swap
                return [teamAssignments[i][j],i,j];
              }
            }          
          }
  ...
  var findMoveAgg = function(team, valueToFind) {
    var aTeam = [];
    for (var i=0; i < teamAssignments.length; i++) {      // Loop teamss
      aTeam[i] = 0;
      for(var j=0; j < teamAssignments[i].length; j++) {  // Loop studentss
        ...
        if(i != team && responses[teamAssignments[i][j]].length > 0 && responses[teamAssignments[i][j]].indexOf(valueToFind) > -1) { 
          aTeam[i]++; 
        }
      }
      if(aTeam[i] > 2) return i;                          // If they've more than one value it's a possible team
    }
  ...
  var findSwapBalance = function(i,team, weight) {
    ...
    for (var j=0; j < teamAssignments.length; j++) {            // Loop teams
      for(var k=0; k < teamAssignments[j].length; k++) {        // Loop students
        if(teamAvg[j] < average && weight == 0 && j != team) {  // Look for a student with a value under the average and not in the requesting team !
          for(var l=0; l < aNeeded.length; l++) {               // Loop through values
            if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[l],10)) != -1)  {
              // If student result is heavier than the average find a lighter one...
              if(parseInt(questions[aCommand[i][2]].answers[parseInt(aNeeded[l],10)].answer,10) <  average && aDontMove.indexOf(teamAssignments[j][k]) == -1) { 
                return [teamAssignments[j][k],j,k];             // Return the id of the student, and the index of is team and is position in the team
              } else {
                if(bAgg == true && typeof aTeamMaxAgg[0] !== 'undefined' && aTeamMaxAgg[team][qAgg] ==  aTeamMaxAgg[j][qAgg] && parseInt(questions[aCommand[i][2]].answers[parseInt(aNeeded[l],10)].answer,10) <  average && aDontMove.reduce(function(n, val) {return n + (val === teamAssignments[j][k]);}, 0) == 1) {
                  return [teamAssignments[j][k],j,k];             // Return the id of the student, and the index of is team and is position in the team
                }  
              }
            }
  ...
  var registerEventListeners = function() {
    ...
    $('#buildteams').on('click',      function() {$('#protectAll').modal('show'); setTimeout(function() {buildTeams(true,false);buttonManager(); $('#protectAll').modal('hide');}, 250);});
    $('#resetteams').on('click',      function() {resetTeams();buttonManager();updateTeams(0);updateTeams(1);$('#nbteam').val(1)});
    $('#replay').on('click',          function() {$('#protectAll').modal('show'); setTimeout(function() { buildTeams(false,false);buttonManager();$('#protectAll').modal('hide');}, 250);});
    $('#assignrandomly').on('click',  function() {assignRandomly();buttonManager();});
    $('#creategroups').on('click',    function() {createGroups();buttonManager();});
    $('#prettify').on('click',        function() {$('#protectAll').modal('show'); setTimeout(function() { buildTeams(false,true);buttonManager(); $('#protectAll').modal('hide');}, 250);});
    $('#serie').on('click',           function() {$('#protectAll').modal('show'); setTimeout(function() { buildTeamsAlpha();buttonManager(); $('#protectAll').modal('hide');}, 250);});
    $('#dsp_sum_switch').on('click',  function(event) {$('.box_ok').toggle(); event.stopPropagation();});
    $('#aggList').on('click',         function() { filterOnAgg();});
    $('#equalize').on('click',        function() { equalize();});
    $('#deleteallred').on('click',    function() { deleteallred();});
    $('#keepallred').on('click',      function() { keepallred();});
    /*
    $('#hist_save').on('click',       function() { historySave();});
    $('#hist_go').on('click',         function() { historyGo(0);});
    */
  };
  //*********************************************************************************************************************************************
  // This function is used to check if a value is good or not based on basic rules in the commands list
  // aData contain the id of the answer of the students of the group (id) and the number of student that answer that in the group
  //*********************************************************************************************************************************************
    ...
    for(var i=0;i < aCommand.length;i++) {                            // Loop criteria of the command list
      switch(aCommand[i][0]) {                                        // Type of critetia
        case strings.cluster    :                                     // At least 2 or 0
          var valueArray = aCommand[i][1].split(",");                 // List of the values that cannot be alone (usualy gender, race, minority, etc)
          for(var j=0; j<valueArray.length; j++) {           
            if(aData.id == parseInt(valueArray[j],10)) {
              count = getCountForAllSelected(i, team);                // Recompute the number on answer in case of multiple criteria check ex (cluster FR=NL in GroupEN)
              if(count >= 2) return 'color:green;font-weight:bold;';  
              if(count == 1) return 'color:red;font-weight:bold;';    // Someone is alone...
            }
          } 
        break;
        case strings.distribute :                                     // At least 1 of the selected value must be in each team HETEROGENITY
          ...
          for(var j=0; j<valueArray.length; j++){           
            if(aData.id == parseInt(valueArray[j],10) &&  aData.count > 0)  return 'color:green;font-weight:bold;';
            if(aData.id == parseInt(valueArray[j],10) &&  aData.count == 0) return 'color:red;font-weight:bold;';
          } 
        break;
        case strings.aggregate  :                                     // The value must be alone > 0 the rest of the answers representation must  be 0 HOMOGENICITY
          if ((','+aCommand[i][1]+',').indexOf(','+aData.id+',') != -1) { // really uggly patch... .try split or map
            if (typeof aTeamMaxAgg[team] !== 'undefined') {           // The group with the most representation is OK
              if(aData.id == aTeamMaxAgg[team][i]) return 'background-color:green;color:white;font-weight:bold;font-size:14px;padding:3px;';  
              if(aData.id != aTeamMaxAgg[team][i] &&  aData.count == 0) return 'color:green;font-weight:bold;';
              if(aData.id != aTeamMaxAgg[team][i] &&  aData.count > 0)  return 'color:red;font-weight:bold;';
            }
          }
        break;
        case strings.balance    :                                     // Tolerate a difference of the gloabal average of the half of the standard deviation
          if(aData.id == -1) {
            if(teamAvg[team] >= average-(ecartType/2) && teamAvg[team] <= average+(ecartType/2) ) return 'color:green;font-weight:bold;'; 
  ...
  var ComputeGroupAvg = function() {
    ...
    if(aNeeded.length == 0) return;                     // If not return
    ...
    for (j=0; j < teamAssignments.length; j++) {        // Loop teams
      ...
      nbStu       = 0;
      for(k=0; k < teamAssignments[j].length; k++) {    // Loop students
        for(l=0; l < aNeeded.length; l++) {             // Loop requested values
          if(responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(parseInt(aNeeded[l],10)) != -1)  {
  ...
  var getCountForAllSelected = function (index, team) {
    ...
    for(var x=0;x<teamAssignments[team].length;x++) {
      for(var l=0; l < aNeeded.length; l++) {
        if(responses[teamAssignments[team][x]] != false  && responses[teamAssignments[team][x]].indexOf(parseInt(aNeeded[l],10)) != -1) {
          nCount++;
        }
      }
    }
  ...
  var checkCriterionValidity = function() {
    ...
    if(criterionGroup.length == 0) return true;
    ...
    for (c in criterionGroup) {
      ...
      if(aMyCommand[1].length > 0 && aMyCommand[0] != strings.balance) return true;
