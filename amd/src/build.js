...
define(['jquery', 'jqueryui', 'core/str'], function($, jqui, str) {
        ...
        switch(boolOp) {
            ...
            case strings.aggregate:
                ...
                for (var j=0; j<aAnswer.length; j++) {
                    ...
                    if (reste != 0) {
                        color='red';
                        strText = strText + strings.analyzeaggregatewarning.replace('{color}',color).replace('{answer}',
                            questions[c.question].answers[aAnswer[j]].answer).replace('{nbstudent}',lctr).replace('{reste}',reste)
                                .replace('{nbgroup}',nbAggTeam).replace('{nbstudentgroup}',studAvgByTeam).replace('{reste}',reste);
                    } else {
                        color='green';
                        strText = strText + strings.analyzeaggregatewarningOK.replace('{color}',color).replace('{answer}',
                            questions[c.question].answers[aAnswer[j]].answer).replace('{nbstudent}',lctr).replace('{reste}',reste)
                                .replace('{nbgroup}',nbAggTeam).replace('{nbstudentgroup}',studAvgByTeam).replace('{reste}',reste);
                    }
                }          
            ...
            case strings.distribute:
                ...
                for (var j=0;j<aAnswer.length;j++) {
                    ...
                    strText = strText + strings.analyzedistributecriterion.replace('{answer}', 
                            questions[c.question].answers[aAnswer[j]].answer).replace('{nbstudent}', ctr).replace('{status}', status);
                }          
                break;
                            ...
                            if ($.inArray(parseInt(a.id), myResponses) != -1) {
                                for (cmdIdx=0;cmdIdx < aCommand.length; cmdIdx++) {
                                    if (aCommand[cmdIdx][0] == strings.aggregate && aCommand[cmdIdx][2] == q.id) {
                                        if (typeof aTeamMaxAgg[i] !== 'undefined' && aTeamMaxAgg[i][cmdIdx] == aSum[k].answers[m].id) {
    ...
    var buildTeams = function(bReset, bPrettify) {
        ...
        for (c in criterionGroup) {
            ...
            if (aCommand[i][0] == strings.aggregate) {
                ...
                for (qa in questions[criterion['question']].answers) {
                    aCommand[i][1] = aCommand[i][1] + qa + ',';
                    $('#aggList').append($('<option>', { value: questions[criterion['question']].answers[qa].answer.split(' ').join('_')
        ...
        for (var j=0; j < teamAssignments.length; j++) { // Loop teams
            ...
            if (bPrettify == true) { // Try to prettify the group by moving values that are not the dominant
                ...
                for (var k=0; k < teamAssignments[j].length; k++) {
                    if (responses[teamAssignments[j][k]].length > 0 && responses[teamAssignments[j][k]].indexOf(aTeamMaxAgg[j][i]) == -1 
                            && aDontMove.indexOf(teamAssignments[j][k]) == -1)  { // the student don't have this value and can be moved
                        var nAnswer = -1;
    ...
    var distribute = function(bPrettify) {
        for (var j=0; j < teamAssignments.length; j++) { // Loop teamss
            var aNeeded =  aCommand[i][1].split(","); // List of needed value
            for (var k=0; k < teamAssignments[j].length; k++) { 
